<template>
  <a :href="service.Link" class="transition-transform hover:scale-[1.025]">
    <div class="flex items-center gap-4 rounded-2xl bg-white/90 shadow-sm hover:shadow-lg p-4 min-w-[260px] max-w-md">
      <ServiceIcon :icon="service.Icon" />
      <div class="flex-1">
        <h2 class="text-lg font-semibold mb-2">{{ service.Title }}</h2>
        <span class="flex flex-wrap gap-2">
          <ServiceTag :tag="tag" v-for="tag in serviceTags" :key="tag.Name" />
        </span>
      </div>
    </div>
  </a>
...
const serviceTags = !props.service.Tags ? [] : props.service.Tags
  .map(name => props.tags.find(tag => tag.Name === name))
